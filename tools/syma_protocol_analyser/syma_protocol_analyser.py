...
# Inspired from https://github.com/chopengauer/nrf_analyze
...
regex_address = re.compile(r"^"+filter_address)
...
try:
    ...
    while (running):
        if isData():
            if (1):
                ...
                if c == '\x20':
                    ...
                    full_scan = True
                    channel = 1
                    full_scan_channel_change_trigger = 10
                else:
                    print " 'q' - Quit\n ' ' - Info\n 'w' - channel++\n 's' - channel--\n 'z' - AutoAck\n '0' - clear Stat\n 'r' - change data rate\n 'e' - use next address\n 'a' - set address\n 'p' - set payload len\n 'c' - change crc\n '2-5' use address lenght"
                setup(channel, address[address_index], data_rate[data_rate_index], crc[crc_index])
        ...
        while(radio.available()):
            if (full_scan and full_scan_channel_change_trigger == 0):
                channel=channel+1
                full_scan_channel_change_trigger = 10
            ...
            if(regex_address.match(str(data).encode('hex')) is not None):
                print "channel " + str(channel) + " " + hex_data
            try:
            ...
            if (full_scan):
                full_scan_channel_change_trigger=full_scan_channel_change_trigger-1
finally:

...
3. Lancer un programme permettant de contrôler le drone grâce à la machine ayant perpetré 
   l'attaque.
...
2. **Effectuer un scan actif des réseaux WiFi.** Le scan actif consiste en l'envoi de 
   paquets *Probe Request* en indiquant un SSID. Si l'AP correspondant à cet SSID reçoit 
   ...
   répondent avec un paquet *Probe Response*.
...
notre utilisation de `aireplay-ng` par Scapy. Pour cela, nous avons tout d'abord étudier 
...
> cartes WiFi. Ceci pourrait expliquer l'envoi répété des paquets. Cette découverte peu 
> avant la fin du projet, nous n'avons pas eu le temps de la confirmer/infirmer.
...
connaissances, nous avons souhaité intégrer le passage en mode *monitor* directement le 
